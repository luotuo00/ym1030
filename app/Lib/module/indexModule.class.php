...
class indexModule extends SiteBaseModule
{
	public function index()
	{
		...
		{	
			...
			if($art_id > 0){
				$news_pics = $GLOBALS['db']->getRow("SELECT * FROM  ".DB_PREFIX."article where cate_id=".$art_id." and icon<>'' and is_effect=1 and is_delete=0");
				$GLOBALS['tmpl']->assign("news_pics",$news_pics);

				$compnay_active_list  = get_article_list(6,$art_id);
			...
			if($mtbd_id > 0){
				...
				$mtbd_pics = $GLOBALS['db']->getAll("SELECT * FROM  ".DB_PREFIX."article where cate_id=".$mtbd_id." and icon<>'' and is_effect=1 and is_delete=0 limit 8");

				$GLOBALS['tmpl']->assign("mtbd_pics",$mtbd_pics);

				$mtbd_list  = get_article_list(10,$mtbd_id);
