...
$(document).ready(function(){
	$("#sku label").click(function(){
		...
		if($(this).prop("checked") == "undefined" || $(this).prop("checked") == undefined){
			$(this).find(".mt").attr("checked",true);
		}
		else{
			$(this).find(".mt").prop("checked",false);
		}
		
		var t_score = parseFloat(sum_score);
		$("#sku .Other_Property .mt").each(function(){
			
			if($(this).attr("checked")=="checked" || $(this).attr("checked")==true || $(this).prop("checked")=="checked" || $(this).prop("checked")==true){
				
				var ss_score = 0;
				if($(this).prop("rel") == "undefined" || $(this).prop("rel") == undefined)
					ss_score = $(this).attr("rel");
				else
					ss_score = $(this).prop("rel");
			 
				t_score += parseFloat(ss_score);
			}
		});
		$(".score").html(t_score);
		eachattr();
