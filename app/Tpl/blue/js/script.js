...
function collect_deal(o,deal_id,func)
{
	...
	$.ajax({
		...
		success: function(obj){
			if(obj.open_win == 1)
			{
				$.weeboxs.open(obj.html, {contentType:'text',showButton:false,title:LANG['PLEASE_LOGIN_FIRST'],width:1000,type:'wee'});
			}
			else
			{
				if(func!=null)
...
{	
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=ajax_login", {boxid:'pop_user_login',contentType:'ajax',showButton:false,title:LANG['PLEASE_LOGIN_FIRST'],width:1000,type:'wee',onopen:function(){
		init_ui_checkbox();	
		init_ui_textbox();
	},onclose:func});
}
