...
/*
    美圣短信管理后台 http://112.74.76.186:8040/msm/
    账号'JSM40942';
    密码  'yftr5zn4';
    验证手机号 15261887199
    状态码说明
    ...
    108	余额不足
    109	ip错误
    110#代开发票	短信内容存在系统保留关键词，可以登录客户端，查找具体的敏感词。
    114	模板短信序号不存在
    115	短信签名标签序号不存在
    116	认证码不正确
    117	未开通此接入方式
*/
function ms_sms($method,$mobile="",$content="test",$msgtype=2)
{
    ...
    $svr_param['username'] = 'JSM40942';    // 账号
    $svr_param['password'] = 'yftr5zn4';    // 密码
    $svr_param['veryCode'] = '5coo0hxoj8ra';    // 通讯认证Key
    $post_data = $svr_param;
    ...
    if($method == "sendMsg")
    {
        ...
        if($msgtype == 1)
        {
            $post_data['content']= $content; //自定义内容
            $post_data['msgtype']= '1';       // 1-普通短信，2-模板短信
        }elseif($msgtype == 2)
        {
            $post_data['content']= '@1@='.$content; //随机验证码
            $post_data['msgtype']= '2';             // 1-普通短信，2-模板短信
            $post_data['tempid'] = 'JSM40942-0001'; // 模板编号
        }
        $res = request_post($svr_url, $post_data);
        $ms_arr = xml_to_array($res);//Array([sms] => Array([mt] => Array([status] => 0[msgid] =>727b564e6f214fbf842162a616b49044)))
    }
