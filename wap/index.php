...
if($user_info && $class == 'uc_incharge' &&  $GLOBALS['db']->getOne("select * from ".DB_PREFIX."user_bank where user_id = ".$user_info['id'])==0){
	showErr("请先绑定一张银行卡",0,wap_url('index','uc_bank'));
}
if($user_info && $class == 'uc_add_bank' &&  !$user_info['idno']){
	showErr("请先进行实名认证",0,wap_url('index','uc_credit'));
}
